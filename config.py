    ...
    if DATABASE_URL:
        ...
        if DATABASE_URL.startswith('mssql://'):
            ...
            if '?' not in SQLALCHEMY_DATABASE_URI:
                SQLALCHEMY_DATABASE_URI += '?driver=ODBC+Driver+17+for+SQL+Server&Encrypt=yes&TrustServerCertificate=no&Connection+Timeout=30'
            else:
                SQLALCHEMY_DATABASE_URI += '&Encrypt=yes&TrustServerCertificate=no&Connection+Timeout=30'
        else:
